...
class Ps2_CheckPassword{
	...
	public function check() {
		...
		if (strlen($this->_password) < $this->_minimumChars) {
			$this->_errors[] = "Password must be at least $this->_minimumChars charachters.";
		}
		if ($this->_mixedCase) {
			$pattern = '/(?=.*[a-z])(?=.*[A-Z])/';
			if (!preg_match($pattern, $this->_password)) {
				$this->_errors[] = 'Password should include uppercase and lowercase charachters.';
			}
		}
		...
		if ($this->_minimumSymbols) {
			...
			if ($found < $this->_minimumSymbols) {
				$this->_errors[] = "Password should include at least $this->_minimumSymbols nonalphanumeric charachter(s).";
			}
		}
